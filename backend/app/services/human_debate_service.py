...
def gather_sources_background(session_id: str, topic: str):
    """Sync background task: search + index sources for both sides."""
    from app.database import HumanDebate, SessionLocal
...
async def stream_ai_opening(session_id: str, topic: str, ai_side: Side, db):
    ...
    try:
        chunks = await loop.run_in_executor(
            None, retrieve, session_id, ai_str, topic, 3
        )
        ...
        async for kind, data in generate_argument_streaming(
            topic, ai_side, "opening", chunks
        ):
            if kind == "token":
                yield _sse(
                    "token", {"side": ai_str, "round_name": "opening", "delta": data}
                )
            else:
...
async def stream_ai_rebuttal_and_verdict(
    ...
    try:
        # ── AI rebuttal ───────────────────────────────────────────────────
        chunks = await loop.run_in_executor(
            None, retrieve, session_id, ai_str, human_rebuttal, 3
        )
        ...
        async for kind, data in generate_argument_streaming(
            ...
            if kind == "token":
                yield _sse(
                    "token", {"side": ai_str, "round_name": "rebuttal", "delta": data}
                )
            else:
        ...
        # ── Judge verdict ─────────────────────────────────────────────────
        yield _sse(
            "status", {"message": "Judge is deliberating...", "step": 5, "total": 5}
        )

        judge_chunks = await loop.run_in_executor(
            None, retrieve, session_id, "pro", topic, 2
        )
        judge_chunks += await loop.run_in_executor(
            None, retrieve, session_id, "con", topic, 2
        )
        ...
        async for kind, data in generate_argument_streaming(
            ...
            if kind == "token":
                yield _sse(
                    "token", {"side": "judge", "round_name": "verdict", "delta": data}
                )
            else:
